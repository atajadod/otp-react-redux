...
export function itineraryToTransitive (itin, includeGeometry, config) {
  ...
  itin.legs.forEach(leg => {
    ...
    if (isTransit(leg.mode)) {
      ...
      let zoomLevel=config.map.initZoom || 13;
      if (!addLegGeometry || zoomLevel<9){
        ...
        var arr=polyline.decode(leg.legGeometry.points);      
        //        
        // var devideFactor=1;        
        // if (arr.length>100)
        //   devideFactor=10;
        ...
        //console.log("mapZoomLevel:",zoomLevel, additionalStopCounter);
        //console.log("distanceFactor:",distanceFactor);

        var lastP=null;
        for(let i=0;i<arr.length;i++){
          //if ((i%devideFactor)!=0) continue;
          //if ((i%7)!=0) continue;
          //if ((i%10)!=0) continue;
          
          let p=arr[i];
          // var dis=1000;
          // if (distanceFactor>0 && arr.length>20){
          //   if (lastP!=null ){
          //     dis=haversine(
          //       { latitude: lastP[0], longitude: lastP[1] },
          //       { latitude: p[0], longitude: p[1] },
          //       {unit: 'meter'}
          //       );
          //     if (dis<=distanceFactor) continue;
          //   }  
          // }
          additionalStopCounter++;
          ...
          let id= leg.from.stopId + ":" + additionalStopCounter;
          stops[id]={
            stop_id: id,
            stop_name: id,
            stop_lat: p[0],
