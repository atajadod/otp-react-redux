...
class DefaultMainPanel extends Component {
  render () {
    ...
    let planDisabled = isEqual(currentQuery, mostRecentQuery)
    // We should enable the Plan if the "NOW" is selected.
    if (planDisabled && currentQuery && currentQuery.departArrive && currentQuery.departArrive==="NOW") planDisabled=false;
    return (
      <ViewerContainer>
        <div style={{
          position: 'absolute',
