...
import { findStopsWithinBBox, clearStops, getAgencyInformationsByFeedId, ClearAgencyInformations } from '../../actions/api'
...
class StopsOverlay extends MapLayer {
  ...
  render () {
    const { leaflet, minZoom, setLocation, setViewedStop, setMainPanelContent, stops, languageConfig, icons, getAgency, clearAgency } = this.props
...
class StopMarker extends Component {
  ...
  render () {
    const { setLocation, stop, languageConfig, icons } = this.props
    ...
    _getAgencyName=async (id)=>{
      return await this.props.getAgency(id)
    }
      ...
      >
        <Popup>
          <div className='map-overlay-popup'>
            ...
            <div className='popup-row'><b>Agency:</b> {_getAgencyName(idArr[0])}</div>
...
const mapStateToProps = (state, ownProps) => {
  return {
    ...
    agencies:state.otp.overlay.transit.agencies,
