...
import { findStopsWithinBBox, clearStops, getAgencyInformationsByFeedId, ClearAgencyInformations, getFeeds } from '../../actions/api'
...
class StopMarker extends Component {
  ...
  _getAgency= (feedId)=>{
    ...
    // if (this.props.agencies && this.props.agencies.feedId && this.props.agencies.feedId==feedId) return;    
    // this.props.clearAgency();
    // this.props.getAgency(feedId);
    // //return "sss_" + id;
  }  
  // _agency=(feedId)=>{
  //   const { agencies } = this.props
  //   return MemoryCache.agencies[feedId] || 
  //     (agencies &&  agencies.agencies && agencies.agencies.length>0 && agencies.agencies[0])
  // }
  render () {
    ...
    return (      
      <Marker
        ...
        icon={icon}
      >
        <Popup onOpen={()=>{this._getAgency(idArr[0])}}>
          <div className='map-overlay-popup'>
            <div className='popup-title'>{name}</div>
            {/* <div className='popup-row'><b>Agency:</b> {agencies &&  agencies.agencies && agencies.agencies.length>0 && (<a href={agencies.agencies[0].url}>{agencies.agencies[0].name}</a>)}</div> */}
