...
function getRoutingParams (otpState, ignoreRealtimeUpdates) {
  ...
  params.numItineraries= config.plan.numItineraries
  return params
}
...
const receivedAgencyInfoByFeedIdResponse = createAction('AGENCY_BY_FEEDID_RESPONSE')
const receivedAgencyInfoByFeedIdError = createAction('AGENCY_BY_FEEDID_ERROR')

// export function getAgencyInformationsByFeedId (feedId) {  
//   return createQueryAction(
//     `index/agencies/${feedId}`,
//     receivedAgencyInfoByFeedIdResponse,
//     receivedAgencyInfoByFeedIdError,
//     {
//       serviceId: 'agencies',
//       rewritePayload: (agency) => {
//         MemoryCache.agencies[feedId]=agency;
//         return {agency,feedId};
//       },
//       noThrottle: true
//     },
//     MemoryCache.agencies[feedId]
//   )
// }
export function getAgencyInformationsByFeedId (feedIds) {  
  let urls=[];
  let cachedObject=null;
  //console.log("------------------getAgencyInformationsByFeedId",MemoryCache.agencies);
  if (MemoryCache.agencies){
    let useCache=true;
    for(let i=0;i<feedIds.length;i++){
      if (!MemoryCache.agencies.hasOwnProperty(feedIds[i])){
        useCache=false;
        break;
      }
    }
    if (useCache)cachedObject=MemoryCache.agencies;
  }
  for (let i=0;i<feedIds.length;i++)
    urls.push(`index/agencies/${feedIds[i]}`,)
  return createQueryActions(
    ...
    {
      serviceId: 'agencies',
      rewritePayload: (agencies) => {
        let res={};
        agencies.forEach((agency)=>{
          let eps=agency.endpoint.split('/');
          let feedId=eps[eps.length-1];
          let v={feedId:feedId, agencies:agency.result};
          res[feedId]=v;
          MemoryCache.agencies[feedId]=v;
        });        
...
const receivedAgenciesInfoResponse = createAction('AGENCIES_RESPONSE')
const receivedAgenciesInfoError = createAction('AGENCIES_ERROR')

export function getAgenciesInformations() {    
  //console.log("getAgenciseInformations called.");
  return createQueryAction(
    `index/agencies`,
    receivedAgenciesInfoResponse,
    receivedAgenciesInfoError,
    {
      serviceId: 'agencies',
      rewritePayload: (agencies) => {
        //console.log("getAgenciseInformations returned.", agencies);
        let res={};
        agencies.forEach((agency)=>{
          let feedId=agency.feedId;;
          //let v={feedId:feedId, agencies:[agency]};
          res[feedId]=agency;
        });        
        //console.log("getAgenciseInformations returned after process.", res);
        return res;
      },
    }
  )
...
export const ClearAgencyInformations = createAction('CLEAR_AGENCY_BY_FEEDID_RESPONSE')

const receivedFeedsResponse = createAction('FEEDS_RESPONSE')
const receivedFeedsIdError = createAction('FEEDS_ERROR')
export function getFeeds () {
  return createQueryAction(
    ...
    findRouteError,
    {
      postprocess: getFeedsPostProcess,
      noThrottle: false
    },
    MemoryCache.feeds
  )
}
function getFeedsPostProcess(payload, dispatch){
  //console.log("------------getFeedsPostProcess", payload);
  let feeds=payload || [];
  MemoryCache.feeds=feeds;  
  dispatch(getAgencyInformationsByFeedId(feeds))
}
...
function createQueryAction (endpoint, responseAction, errorAction, options = {}, cachedData) {
  return async function (dispatch, getState) {
    const otpState = getState().otp
    let url
...
function createQueryActions (endpoints, responseAction, errorAction, options = {}, cachedData) {
  return async function (dispatch, getState) {
    const otpState = getState().otp
    let urls=[]
    if (options.serviceId && otpState.config.alternateTransitIndex &&
      ...
      console.log('Using alt service for ' + options.serviceId)
      endpoints.forEach((endpoint)=>urls.push({url:otpState.config.alternateTransitIndex.apiRoot + endpoint, endpoint:endpoint}));
    } else {
      const api = otpState.config.api
      endpoints.forEach((endpoint)=>urls.push({url:`${api.host}${api.port ? ':' + api.port : ''}${api.path}/${endpoint}`, endpoint:endpoint}));      
    }

    let payload
    //console.log("-------------------------createQueryActions cached data", cachedData);
    if (typeof cachedData==='undefined' || cachedData==null){
      try {
        let results=[];
        let results_raw=[];
        ...
        for(let i=0;i<urls.length;i++){
          let urlInfo=urls[i];          
          promises.push(fetch(urlInfo.url, options.fetchOptions));
          //console.log("----------------promises " + i, `prom count:${promises.length}, resultCount: ${results_raw.length}`);
          if (i%5==0){  //Maximum 5 every time.
            let responses=await Promise.all(promises);
            //console.log("-----------------responses", responses);
            Array.prototype.push.apply(results_raw,responses);
            
            //console.log("-----------------responses->result_raw", results_raw);
            promises=[];
          }
        } 
        ...
        let errorStr="";
        for(let i=0;i<urls.length;i++){
          //console.log("----------------urls " + i, promises);
          let urlInfo=urls[i]; 
          let response= results_raw[i];
          if (response.status >= 400)
            errorStr=`${urlInfo.endpoint} \r\n`;
          else     
            results.push({endpoint:urlInfo.endpoint, result:await results_raw[i].json()});
        }        
        if (errorStr!=""){
          const error = new Error(`Received error from server for ${errorStr}`)
          error.response = response
          throw error
        }
        payload = results;
